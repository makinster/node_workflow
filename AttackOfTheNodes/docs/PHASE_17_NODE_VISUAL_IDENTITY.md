...
Implementation note, audited 2026-06-14: `NodeFactory.get_node_types_metadata()`
now exposes the portable identity metadata needed by the frontend: category /
primary family, legacy category, subcategory tags, icon name, and color hint.
Selector filters and editor row identity are built from that metadata.
...
to render each node as an individual ASCII-bordered text box through Textual
styling, with plain node text inside. Avoid literal bracket characters around
the alias or identity line; the border should carry the visual grouping.

The important requirements are:

- node rows may use two lines when space allows;
- border color or frame style can differ by primary family;
- each node should read as its own text box rather than relying on inline
  bracket characters;
...
- Selector tabs, filter state, row borders, row colors, and display density are
  frontend concerns.
...
Current implementation note, audited 2026-06-14: the backend `RunSession` exists
and `FileReaderNode` uses it. The validator also checks schema fields with
`path_hint: "file"`. The missing frontend piece is a generic node-config file
picker or browse affordance for fields with that schema hint; those fields still
render as ordinary text inputs today.

## Current Frontend Support Gaps

These gaps were identified while reviewing backend features against the Textual
frontend. They are not all identity-rendering work, but they should be tracked
while Phase 17 remains active so the roadmap does not claim the control room
fully exposes the factory floor.

- Run history is backend-supported but not browsable in the UI. `RunHistory`,
  `OutputManager`, and `ErrorHandler` can persist and lazy-load historical run
  summaries, outputs, errors, and timings, but the execution screen only exposes
  the current run.
- File/path schema hints are not rendered with picker controls in node config.
  `FileReaderNode.file_path` declares `path_hint: "file"`, and validation uses
  that hint, but `NodeConfigScreen` / `form_generator` still render it as a
  plain text field.
- Workflow memory snapshot save/load options exist in `SaveManager` but have no
  UI. Decide whether `include_memory` / `restore_execution` are product
  features to expose or dormant hooks to keep internal.
- Workflow rename, cached open-workflow switching, and bookmarks exist in
  `WorkflowMap` but are not exposed by the workflow library/editor UI.
- Persisted run errors can be cleared through `ErrorHandler.clear_errors_for_run`
  but there is no frontend action for it.

Do not solve these by moving UI concerns into backend services. If a gap needs
new backend metadata, keep it portable and frontend-neutral.
